...
namespace F4SE
{
	...
	public:
		...
		[[nodiscard]] std::string_view GetSaveFolderName() const { return (F4SEVersion() >= REL::Version(0, 7, 1, 0) && GetProxy().GetSaveFolderName() && !GetProxy().GetSaveFolderName()[0]) ? GetProxy().GetSaveFolderName() : "Fallout4"sv; }
