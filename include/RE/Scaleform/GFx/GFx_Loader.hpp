...
namespace RE::Scaleform::GFx
{
	...
	{
	public:
		inline static constexpr auto RTTI{ RTTI::Scaleform__GFx__State };
		inline static constexpr auto VTABLE{ VTABLE::Scaleform__GFx__State };
	...
	{
	public:
		inline static constexpr auto RTTI{ RTTI::Scaleform__GFx__Translator };
		inline static constexpr auto VTABLE{ VTABLE::Scaleform__GFx__Translator };
	...
	public:
		...
		explicit Translator(SFFlags<WordWrappingType> a_wwMode) :
			State(StateType::kTranslator), wwMode(a_wwMode)
		{
		}

		// members
		SFFlags<WordWrappingType> wwMode; // 18
	};
	...
	public:
		...
		ActionControl(SFFlags<ActionControlFlags> a_actionFlags = ActionControlFlags::kLogChildFilenames) :
			State(StateType::kActionControl), actionFlags(a_actionFlags)
		{
		}

		// members
		SFFlags<ActionControlFlags> actionFlags; // 18
	};
	...
	public:
		...
		// members
		SFFlags<FileTypeConstants::FileFormatType> format; // 00
		const char* prefix;								   // 08
		const char* swfName;							   // 10
		std::uint16_t version;							   // 18
		SFFlags<ExportFlagConstants> exportFlags;		   // 1C
	};
	...
	public:
		...
		[[nodiscard]] MovieDef* CreateMovie(const char* a_filename, SFFlags<LoadConstants> a_loadConstants = LoadConstants::kAll, std::size_t a_memoryArena = 0)
		{
			using func_t = decltype(&Loader::CreateMovie);
			static REL::Relocation<func_t> func{ ::Scaleform::ID::GFx::Loader::CreateMovie };
		...
		SFFlags<LoadConstants> defLoadFlags; // 18
	};
