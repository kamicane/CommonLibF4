...
namespace RE
{
	...
	static_assert(sizeof(BGSLocationLoadedEvent) == 0x8);
	...
	namespace LevelIncrease
	{
		struct Event
		{
			[[nodiscard]] static BSTEventSource<Event>* GetEventSource()
			{
				...
				static REL::Relocation<func_t> func{ ID::LevelIncrease::GetEventSource }; // .163
				return func();
			}
	...
	public:
		...
		// members
		TESFormID cellID;							  // 04
		REX::EnumSet<CellFlags, std::uint32_t> flags; // 08
	};
	...
	public:
		...
		REX::Enum<EVENT_TYPE, std::uint32_t> type; // 08
	};
	...
	namespace CellAttachDetachEventSource
	{
		...
		public:
			[[nodiscard]] static CellAttachDetachEventSourceSingleton& GetSingleton()
			{
				...
				static REL::Relocation<func_t> func{ ID::CellAttachDetachEvent::CellAttachDetachEventSourceSingleton::GetSingleton };
				return func();
			}
	...
	public:
		[[nodiscard]] static EventSource_t* GetEventSource()
		{
			static REL::Relocation<EventSource_t**> singleton{ REL::RelocationID(1536643, 2694310) }; // skip
			if (!*singleton) {
				*singleton = new EventSource_t(&BSTGlobalEvent::GetSingleton()->eventSourceSDMKiller);
			}
			return *singleton;
	...
	public:
		...
		REX::Enum<QuickContainerMode, std::uint32_t> mode;	 // AC
	...
	struct TESCombatEvent
	{
		...
		REX::Enum<ACTOR_COMBAT_STATE, std::uint32_t> setState; // 10
	...
	public:
		...
		REX::Enum<FurnitureEventType, std::uint32_t> type; // 10
	};
	...
	public:
		// members
		REX::EnumSet<UserEvents::USER_EVENT_FLAGS, std::int32_t> newUserEventFlag; // 00
		REX::EnumSet<UserEvents::USER_EVENT_FLAGS, std::int32_t> oldUserEventFlag; // 04
		REX::Enum<UserEvents::SENDER_ID, std::uint32_t> senderID;				   // 08
	};
	...
	public:
		...
		{
			static REL::Relocation<TESInitScriptEventSource*> singleton{ REL::RelocationID(0, 2691330) }; // skip
			return singleton.get();
		}
	};
}
