...
namespace RE
{
	...
	namespace BGSInventoryListEvent
	{
		...
		public:
			// members
			REX::Enum<Type, std::uint16_t> changeType; // 00
	...
	public:
		...
		BSContainer::ForEachResult ForEachStack(std::function<BSContainer::ForEachResult(BGSInventoryItem::Stack&)> a_callback) const;
	...
	public:
		...
		void* handleList;				  // 38 - To-Do: BSModelDB::HandleListHead
		union
		{
			std::byte spare40;
	...
	public:
		...
		struct RecordFlags
		{
			...
			{
				kIsGroundPiece = 1u << 4,
				kCollisionsDisabled = 1u << 4, // TESObjectREFR
				kDeleted = 1u << 5,
				kHiddenFromLocalMap = 1u << 6, // TESObjectSTAT
				kTurnOffFire = 1u << 7,
				kInaccessible = 1u << 8,		   // TESObjectDOOR
				kLODRespectsEnableState = 1u << 8, // TESObjectSTAT
				kStartsDead = 1u << 8,			   // TESNPC
				kDoesNotLightWater = 1u << 8,
				kMotionBlur = 1u << 9, // TESObjectSTAT
				kPersistent = 1u << 10,
				kInitiallyDisabled = 1u << 11,
				kIgnored = 1u << 12,
				kStartUnconscious = 1u << 13, // TESNPC
				kSkyMarker = 1u << 13,
				kHarvested = 1u << 13,	// TESProduceForm
				kIsFullLOD = 1u << 16,	// Actor
				kNeverFades = 1u << 16, // TESObjectLIGH
				kDoesNotLightLandscape = 1u << 17,
				kIgnoreFriendlyHits = 1u << 20, // Actor
				kNoAIAcquire = 1u << 25,
				kCollisionGeometry_Filter = 1u << 26,
				kCollisionGeometry_BoundingBox = 1u << 27,
				kReflectedByAutoWater = 1u << 28,
				kDontHavokSettle = 1u << 29,
				kGround = 1u << 30,
				kRespawns = 1u << 30,
				kMultibound = 1u << 31
			};
		...
		[[nodiscard]] TESBoundObject* GetBaseObject() const noexcept { return data.objectReference; }

		[[nodiscard]] void SetBaseObject(TESBoundObject* a_object) { SetObjectReference(a_object); }
		...
		[[nodiscard]] const NiPoint3A GetPosition() const noexcept { return data.location; }

		void SetPosition(const NiPoint3A& a_point) noexcept { data.location = a_point; }

		[[nodiscard]] const NiPoint3A GetRotation() const noexcept { return data.angle; }

		void SetRotation(const NiPoint3A& a_point) noexcept { data.angle = a_point; }
		...
		[[nodiscard]] bool IsInWater()
		{
			...
			static REL::Relocation<func_t> func{ REL::ID(315318) }; // skip
			return func(this);
		}
	...
	public:
		...
		void* explosionDBHandle;						// 520 - To-Do
		float age;										// 528
	...
	public:
		...
		inline static constexpr auto FORMTYPE{ ENUM_FORMTYPE::kPHZD };
