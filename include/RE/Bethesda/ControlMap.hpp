...
namespace RE
{
	...
	public:
		...
		public:
			...
			bool remappable;															 // 0D
			bool linked;																 // 0E
			REX::EnumSet<UserEvents::USER_EVENT_FLAGS, std::int32_t> userEventGroupFlag; // 10
		};
		...
		public:
			...
			REX::Enum<UserEvents::INPUT_CONTEXT_ID, std::uint32_t> linkedMappingContext; // 08
			REX::Enum<INPUT_DEVICE, std::int32_t> device;								 // 0C
			REX::Enum<UserEvents::INPUT_CONTEXT_ID, std::uint32_t> linkFromContext;		 // 10
		...
		BSTArray<REX::Enum<UserEvents::INPUT_CONTEXT_ID, std::uint32_t>> contextPriorityStack; // 128
		std::int8_t byTextEntryCount;														   // 140
		bool ignoreKeyboardMouse;															   // 141
		bool ignoreActivateDisabledEvents;													   // 142
		REX::Enum<PC_GAMEPAD_TYPE, std::uint32_t> pcGamePadMapType;							   // 144
	};
