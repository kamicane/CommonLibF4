...
namespace RE
{
	...
	public:
		// members
		const char* name;									  // 00
		REX::Enum<ENUM_FORMTYPE, std::uint8_t> type;		  // 08
	...
	{
		static REL::Relocation<DEFAULT_OBJECT_DATA(*)[394]> data{ RE::ID::DEFAULT_OBJECT_DATA::GetDefaultObjectData };
		return { *data };
	}
	...
	public:
		...
		inline static constexpr auto FORMTYPE{ ENUM_FORMTYPE::kDOBJ };
		...
		[[nodiscard]] TESForm* GetDefaultObject(DEFAULT_OBJECT a_obj) const noexcept
		{
			...
			return objectInitArray[std::to_underlying(a_obj)] ? objectArray[std::to_underlying(a_obj)] : nullptr;
		}
	...
	public:
		...
		inline static constexpr auto FORMTYPE{ ENUM_FORMTYPE::kDFOB };

		BGSDefaultObject(const char* a_name, ENUM_FORMTYPE a_formType, const char* a_description)
		{
		...
		// members
		TESForm* form;				// 20
		ENUM_FORMTYPE type;			// 28
		BSFixedString formEditorID; // 30

	private:
		BGSDefaultObject* ctor(const char* a_name, ENUM_FORMTYPE a_formType, const char* a_description)
		{
