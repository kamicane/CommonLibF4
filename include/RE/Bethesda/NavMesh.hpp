...
namespace RE
{
	...
	public:
		// members
		REX::Enum<EDGE_EXTRA_INFO_TYPE, std::uint32_t> type; // 00
	...
	{
	public:
		inline static constexpr auto RTTI{ RTTI::BSNavmesh };
		inline static constexpr auto VTABLE{ VTABLE::BSNavmesh };

		struct DisabledLink
		{
		public:
			// members
			std::uint16_t triIndex;	   // 00
			std::uint16_t adjTriIndex; // 02
			std::int32_t edgeIndex;	   // 04
			std::int32_t adjEdgeIndex; // 08
		};
	...
	public:
		...
		inline static constexpr auto FORMTYPE{ ENUM_FORMTYPE::kNAVM };
	...
	public:
		...
		{
			static REL::Relocation<DynamicNavmesh**> singleton{ REL::RelocationID(908540, 2690147) }; // skip
			return *singleton;
		}
		...
		void ForceUpdate()
		{
			...
			static REL::Relocation<func_t> func{ REL::RelocationID(854653, 2301428) }; // skip
			return func(this);
		}
	};
}
