...
namespace RE
{
	...
	namespace Interface3D
	{
		...
		public:
			...
			public:
				// members
				REX::Enum<LightType, std::uint32_t> type; // 00
				float spotFOV;							  // 04
			...
			REX::Enum<BackgroundMode, std::uint32_t> bgmode;			// 05C
			REX::Enum<PostEffect, std::uint32_t> postfx;				// 060
			REX::Enum<HighlightEffect, std::uint32_t> highlightfx;		// 064
			REX::Enum<OffscreenMenuSize, std::uint32_t> omsize;			// 068
			REX::Enum<ScreenMode, std::uint32_t> screenmode;			// 06C
			REX::Enum<OffscreenMenuBlendMode, std::uint32_t> menuBlend; // 070
			REX::Enum<UI_DEPTH_PRIORITY, std::uint32_t> depth;			// 074
