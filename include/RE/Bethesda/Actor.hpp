...
namespace RE
{
	...
	{
	public:
		inline static constexpr auto RTTI{ RTTI::EquippedItemData };
		inline static constexpr auto VTABLE{ VTABLE::EquippedItemData };
	...
	public:
		...
		REX::Enum<MagicSystem::WardState, std::uint32_t> wardState;												 // 0F8
	...
	public:
		...
		TESPackage* GetPackageThatIsRunning()
		{
			...
			static REL::Relocation<func_t> func{ REL::ID(148295) }; // skip
			return func(this);
		}
	...
	public:
		...
		{
			kNone = 0,
			kDelayUpdateScenegraph = 1u << 0,
			kProcessMe = 1u << 1,
			kMurderAlarm = 1u << 2,
			kHasSceneExtra = 1u << 3,
			kHeadingFixed = 1u << 4,
			kSpeakingDone = 1u << 5,
			kIgnoreChangeAnimationCall = 1u << 6,
			kSoundFileDone = 1u << 7,
			kVoiceFileDone = 1u << 8,
			kInTempChangeList = 1u << 9,
			kDoNotRunSayToCallback = 1u << 10,
			kDead = 1u << 11,
			kForceGreetingPlayer = 1u << 12,
			kForceUpdateQuestTarget = 1u << 13,
			kSearchingInCombat = 1u << 14,
			kAttackOnNextTheft = 1u << 15,
			kEvpBuffered = 1u << 16,
			kResetAI = 1u << 17,
			kInWater = 1u << 18,
			kSwimming = 1u << 19,
			kVoicePausedByScript = 1u << 20,
			kWasInFrustrum = 1u << 21,
			kShouldRotateToTrack = 1u << 22,
			kSetOnDeath = 1u << 23,
			kDoNotPadVoice = 1u << 24,
			kFootIKInRange = 1u << 25,
			kPlayerTeammate = 1u << 26,
			kGivePlayerXP = 1u << 27,
			kSoundCallbackSuccess = 1u << 28,
			kUseEmotion = 1u << 29,
			kGuard = 1u << 30,
			kParalyzed = 1u << 31
		};
		...
		enum class BOOL_FLAGS : std::uint32_t
		{
			...
			kScenePackage = 1u << 0,
			kIsAMount = 1u << 1,
			kIsMountPointClear = 1u << 2,
			kIsGettingOnOffMount = 1u << 3,
			kInRandomScene = 1u << 4,
			kNoBleedoutRecovery = 1u << 5,
			kInBleedoutAnimation = 1u << 6,
			kCanDoFavor = 1u << 7,
			kShouldAnimGraphUpdate = 1u << 8,
			kCanSpeakToEssentialDown = 1u << 9,
			kBribedByPlayer = 1u << 10,
			kAngryWithPlayer = 1u << 11,
			kIsTresspassing = 1u << 12,
			kCanSpeak = 1u << 13,
			kIsInKillMove = 1u << 14,
			kAttackOnSight = 1u << 15,
			kIsCommandedActor = 1u << 16,
			kForceOneAnimGraphUpdate = 1u << 17,
			kEssential = 1u << 18,
			kProtected = 1u << 19,
			kAttackingDisabled = 1u << 20,
			kCastingDisabled = 1u << 21,
			kSceneHeadtrackRotation = 1u << 22,
			kForceIncMinBoneUpdate = 1u << 23,
			kCrimeSearch = 1u << 24,
			kMovingIntoLoadedArea = 1u << 25,
			kDoNotShowOnStealthMeter = 1u << 26,
			kMovementBlocked = 1u << 27,
			kAllowInstantFurniturePopInPlayerCell = 1u << 28,
			kForceAnimGraphUpdate = 1u << 29,
			kCheckAddEffectDualCast = 1u << 30,
			kUnderwater = 1u << 31,
		};
		...
		float GetHealthPercent()
		{
			...
			static REL::Relocation<func_t> func{ REL::ID(260817) }; // skip
			return func(this);
		}
		...
		[[nodiscard]] TESNPC* GetActorBase() const noexcept;

		bool GetOfferBarter()
		{
			using func_t = decltype(&Actor::GetOfferBarter);
			static REL::Relocation<func_t> func{ REL::ID(1482717) }; // skip
			return func(this);
		}
		...
		int32_t RequestDetectionLevel(Actor* a_target, DETECTION_PRIORITY a_priority = DETECTION_PRIORITY::kNormal)
		{
			...
			static REL::Relocation<func_t> func{ REL::ID(943772) }; // skip
		...
		REX::Enum<ACTOR_CRITICAL_STAGE, std::uint32_t> criticalStage; // 378
