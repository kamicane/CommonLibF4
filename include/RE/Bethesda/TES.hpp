...
namespace RE
{
	...
	public:
		...
		BSContainer::ForEachResult ForEachRefInRange(const NiPoint3& a_center, float a_radius,
			std::function<BSContainer::ForEachResult(TESObjectREFR* a_ref)> a_callback) const;
		BSContainer::ForEachResult ForEachRefInRange(const TESObjectREFR* a_origin, float a_radius,
			std::function<BSContainer::ForEachResult(TESObjectREFR* a_ref)> a_callback) const;
