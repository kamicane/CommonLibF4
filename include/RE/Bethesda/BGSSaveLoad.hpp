...
namespace RE
{
	...
	public:
		...
		{
			kNone = 0,
			kDefault = 1u << 6, // index 0x00
			kCreated = 1u << 7, // index 0xFF
		};

		[[nodiscard]] bool IsCreated() const noexcept { return flags.any(Flags::kCreated); }
		[[nodiscard]] bool IsDefault() const noexcept { return flags.any(Flags::kDefault); }
		...
		union
		{
			struct
			{
				...
				std::uint8_t data2; // 01
				std::uint8_t data3; // 02
			};
